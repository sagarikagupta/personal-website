...
const Main = () => {
  ...
  return (
    <>
      <ReactParticles />
      <div id='main' className='h-screen bg-gradient-to-r from-blue-300 to-blue-100 relative' style={{ color: "#000000" }}>
        <img className={`w-[400px] rounded-full absolute bottom-60 right-60 mb-0 mr-0 md:block hidden z-10 transition-opacity duration-1000 ${fadeInImage ? "opacity-100" : "opacity-0"
          }`} src={mypicture} alt="" />
        <div className='max-w-[700px] ml-[calc(50%-350px-100px)] h-[90vh] w-full flex flex-col justify-center lg:items-start items-center'>
          <h1 className={`sm:text-6xl text-5xl font-bold text-black transition-opacity duration-1000 ${fadeInH1 ? "opacity-100" : "opacity-0"
            }`}>Hi, I'm <span className='drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]' style={{ color: "#EADFB2" }}>Sagarika</span></h1>
          <h2 className={`flex sm:text-4xl text-3xl pt-4 text-black transition-opacity duration-1000 ${fadeInH2 ? "opacity-100" : "opacity-0"
            }`}>I'm a
            <TypeAnimation
              sequence={[
                'Developer',
